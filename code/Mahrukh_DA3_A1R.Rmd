---
title: "DA3 - Assignment 1 - Mahrukh Khan"
...
data <- read_csv('https://raw.githubusercontent.com/MahrukhKhan/Data_Analysis_3/main/data/morg-2014-emp.csv')
```
...
data_sum <- datasummary( age + female + HC + MA + BA + GOV + PP + PNP + White + wage ~
...
x <- ggplot(data=data, aes(wage)) + 
       ...
       labs(title='Distribution of Wage', x='Earnings Per Hour (US Dollars)', y='Percent') + 
...
a <- ggplot(data = data, aes(x=age, y=lnw)) +
  ...
  labs(title='Lowess with Age', x = "Age (years)",y = "Log Wage") +
  theme_bw() +
  expand_limits(x = 0.01, y = 0.01) +
...
b <- ggplot(data = data, aes(x=age,y=lnw)) +
  ...
  labs(title='Lowess with Age and Age Square', x = "Age (years)",y = "Log  Wage ") +
...
c <- ggplot(data = data, aes(x=age,y=lnw)) +
  ...
  geom_point( aes( y = lnw ) , color = 'darkseagreen', size = 1,  shape = 16, alpha = 0.8, show.legend=F, na.rm = TRUE) + 
  labs(title='Lowess with Age and Age Cube', x = "Age (years)",y = "Log Wage ") +
...
d <- ggplot(data=data, aes(age)) + 
  geom_histogram(aes(y = (..count..)/sum(..count..)), fill = 'darkseagreen4', color = 'black', 
                 size = 0.25, alpha = 0.8, bins = 30) + 
  labs(title='Distribution of Age', x='Age (years)', y='Percent') + 
  scale_y_continuous(expand = c(0.01,0.01),labels = scales::percent_format(accuracy = 1)) +
  scale_x_continuous(expand = c(0.01,0.01),breaks = seq(15,70, 5)) +
  theme_bw()
...
e <- ggplot(data=data, aes(factor(race))) + 
  ...
  labs(title='Distribution of Race', x='Race', y='Percent') + 
  scale_y_continuous(expand = c(0.01,0.01),labels = scales::percent_format(accuracy = 1)) +
...
f <- ggplot(data=data, aes(factor(female))) + 
  geom_histogram(aes(y = (..count..)/sum(..count..)), fill = 'darkseagreen4', color = 'black', 
                 size = 0.25, alpha = 0.8, stat='count') + 
  labs(title='Distribution of Gender', x='Gender', y='Percent') + 
  scale_y_continuous(expand = c(0.01,0.01),labels = scales::percent_format(accuracy = 1)) +
...
h <- ggplot(data=data, aes(factor(class))) + 
  geom_histogram(aes(y = (..count..)/sum(..count..)), fill = 'darkseagreen4', color = 'black', 
                 size = 0.25, alpha = 0.8, stat='count') + 
  labs(title='Distribution of Job Sector', x='Job Sector', y='Percent') + 
  scale_y_continuous(expand = c(0.01,0.01),labels = scales::percent_format(accuracy = 1)) +
...
i <- ggplot( m_comp , aes( x = complexity , y = RMSE ) ) +
  ...
  labs(x='Number of explanatory variables',y='Averaged RMSE',
...
- Earnings per hour was set between 1 and 100 dollars. 
...
- All variables' distributions were made for a better understanding of the data. Please see figure 4. 
- Functional forms of continuous variable, age, were explored using the lowess curve to analyze its association with the target variable. Please   see figure 5. 

## Figure 1: Variable Description 

```{r, echo=FALSE, warning=FALSE, message=FALSE}
var_data <- read_csv('https://raw.githubusercontent.com/MahrukhKhan/Data_Analysis_3/main/data/DA3-Assignment-Variables.csv')
kable(var_data, format="latex", booktabs=TRUE) %>% kable_styling(latex_options = c('striped','hold_position',"scale_down"), font_size = 4, full_width=F)
```
...
## Figure 5: RMSE AND BIC for full sample

```{r, echo=FALSE}
kable(full_sample, format="latex", booktabs=TRUE) %>% kable_styling(latex_options = c('hold_position',"scale_down"), font_size = 8, full_width=F)
```
